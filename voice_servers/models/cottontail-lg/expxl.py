...
jobbuffera = [*[(torch.zeros(16,8,96, 192, device="cuda", requires_grad=False),) for i in range(layers)],  (torch.zeros(16,8,64, 128, device="cuda", requires_grad=False),)]
...
class Lina(nn.Module):
    def __init__(
        ...
        self.logits_head = nn.Sequential(
            nn.Linear(d_model, self.n_quant*self.n_target_vocab*4, bias=False),
            nn.SiLU(),
            nn.Linear(self.n_quant*self.n_target_vocab*4, self.n_quant*self.n_target_vocab, bias=False),
            nn.SiLU(),
...
modelfile = torch.load("./expxl.pt", map_location="cpu")
modelfile = modelfile
...
async def cloneVoice(request:Request):
    ...
    st = [*[(torch.rand(1,8,96, 192, device="cuda", requires_grad=True),) for i in range(layers)], (torch.rand(1,8,64, 128, device="cuda", requires_grad=True),)]
...
async def run():
    ...
    print("Server started at http://0.0.0.0:8082")
